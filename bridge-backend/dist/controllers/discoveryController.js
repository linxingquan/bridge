...
exports.updateFilters = exports.swipeUser = exports.getDiscoveryFeed = void 0;
...
const getDiscoveryFeed = async (req, res) => {
    try {
        ...
        const swipedUserIds = await models_1.Swipe.find({ fromUserId: user._id }).distinct('toUserId');
        ...
        const feed = potentialMatches.map((u) => ({
            ...
            distance: calculateDistance(myLocation[1], myLocation[0], u.profile.location.coordinates[1], u.profile.location.coordinates[0]),
...
const swipeUser = async (req, res) => {
    try {
        ...
        if (type === 'like' || type === 'superlike') {
            const user = await models_1.User.findById(fromUserId);
            if (user) {
        ...
        const existingSwipe = await models_1.Swipe.findOne({
            fromUserId,
            toUserId,
        });
        if (existingSwipe) {
        ...
        const swipe = new models_1.Swipe({
            fromUserId,
            toUserId,
            type,
        });
        await swipe.save();
        const mutualSwipe = await models_1.Swipe.findOne({
            fromUserId: toUserId,
            toUserId: fromUserId,
...
const updateFilters = async (req, res) => {
    try {
        ...
        const updateFields = {};
        if (ageMin !== undefined)
            updateFields['preferences.ageMin'] = ageMin;
        if (ageMax !== undefined)
            updateFields['preferences.ageMax'] = ageMax;
        if (maxDistance !== undefined)
            updateFields['preferences.maxDistance'] = maxDistance;
        if (distanceUnit !== undefined)
            updateFields['preferences.distanceUnit'] = distanceUnit;
        const user = await models_1.User.findByIdAndUpdate(userId, { $set: updateFields }, { new: true }).select('-password');
        res.json(user);
    }
    catch (error) {
        res.status(500).json({ message: 'Server error' });
